...
void *Database::createRecordFromData(const QSqlQuery &query, const QSqlRecord &rec, std::string tableName)
{
    ...
    else if (tableName == "ACCOUNTS" || tableName == "DEPOSITS")
    {
        ...
        CurrencyType currencyType = (CurrencyType)query.value(rec.indexOf("CURRENCY_TYPE")).toInt();
    ...
    else if (tableName == "CREDITS")
    {
        ...
        CurrencyType currency = (CurrencyType)query.value(rec.indexOf("CURRENCY")).toInt();
